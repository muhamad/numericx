...
**
** Purpose: int_t/uint_t little-endian implementation for int_logic
**
===========================================================*/

#ifndef MATHX_INT_IMP_LE_DEFAULT_LOGIC_HPP
#define MATHX_INT_IMP_LE_DEFAULT_LOGIC_HPP
...
struct default_logic
{
	...
	{
		for (cs_t i = 0; i < spec::comp_c; ++i) if(obj.comp[i] != zero_v) return true;
		return false;
	}
	...
	inline static bool less_than(const type& lhs, const type& rhs)
	{
		if (spec::is_signed)
		{
			...
			if ((st)lhs.comp[spec::comp_c -1] < (st)rhs.comp[spec::comp_c -1]) return true;

			if (neg)
			{
				for (cs_t i=spec::comp_c - 2; i >= 0 ; --i)
...
#endif // MATHX_INT_IMP_LE_DEFAULT_LOGIC_HPP
